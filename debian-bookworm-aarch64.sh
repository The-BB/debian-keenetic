...
INSTALLER=$SCRIPT_DIR/debian-bookworm-12.15-aarch64.tar.gz
...
cp -a $BUILD_DIR/linux-aarch64-3.10/iptables-*/ipkg-install/opt/bin/ip* $ROOT_DIR/opt/bin/
cp -a $BUILD_DIR/linux-aarch64-3.10/iptables-*/ipkg-install/opt/sbin/ip* $ROOT_DIR/opt/sbin/
cp -r $BUILD_DIR/linux-aarch64-3.10/iptables-*/ipkg-aarch64-3.10_kn/iptables/opt $ROOT_DIR

echo 'Adding Debian minimal...'
[ -f debian-bookworm-arm64_clean.tgz ] || wget http://ndm.zyxmon.org/binaries/debian/debian-bookworm-arm64_clean.tgz
[ -f debian-bookworm-arm64_clean.tgz ]
sudo tar -xz -C $ROOT_DIR/opt -f debian-bookworm-arm64_clean.tgz
...
#deb http://ftp.debian.org/debian/ bookworm main non-free contrib
#deb-src http://ftp.debian.org/debian/ bookworm main non-free contrib
#EOF
